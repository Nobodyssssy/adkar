...
/* ═══════════════════════════════════════════════════════════
SEARCH BAR FIX
Moves the bar outside the views once, and aggressively restores
focus after view switches so the mobile keyboard never drops.
═══════════════════════════════════════════════════════════ */
...
if(!results.length){
el.innerHTML = `<div class="adkar-empty"> <div class="adkar-empty-icon">🔍</div> <div>No results.<br><button class="btn-cancel" style="margin-top:14px" onclick="clearFilters()">Clear filters</button></div> </div>`;
} else {
...
// 🚨 CRITICAL FIX: Restore focus to the input immediately after view switch.
// This prevents the mobile keyboard from dropping and allows continuous typing.
...
`<button class="filter-chip ${_searchFilters.grade===k?'on':''}" onclick="toggleFilter('grade','${k}')">${label}</button>`
).join('');
...
const favChip = `<button class="filter-chip ${_searchFilters.favOnly?'on':''}" onclick="toggleFilter('favOnly',true)">★ Favorites only</button>`;
